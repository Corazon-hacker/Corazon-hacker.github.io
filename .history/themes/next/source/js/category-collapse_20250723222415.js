document.addEventListener('DOMContentLoaded', function() {
  ...
  container.querySelectorAll('.category-list-item').forEach(item => {
    ...
    if (childList) {
      ...
      link.addEventListener('click', function(e) {
        ...
        // 显示文章列表
        loadCategoryPosts(categoryName, categoryPath, link);
      });
    }
  });
  ...
  function expandFirstCategory() {
    ...
    // 展开第一个顶级分类
    const firstTopItem = topLevelItems[0];
    let currentItem = firstTopItem;
    
    // 递归展开子分类
    while (true) {
      const childList = currentItem.querySelector('ul');
      if (!childList) break;
      ...
      // 更新图标状态
      const icon = currentItem.querySelector('.category-toggle-icon');
      if (icon) {
      ...
      // 获取第一个子分类
      const firstChild = childList.querySelector('.category-list-item');
      if (!firstChild) break;
    ...
    // 尝试触发最后一级分类的点击
    const lastLevelLink = currentItem.querySelector('.category-list-link');
    if (lastLevelLink) {
      // 添加短暂延迟确保DOM更新完成
      setTimeout(() => {
        lastLevelLink.dispatchEvent(new MouseEvent('click', {
          bubbles: true,
          cancelable: true
        }));
      }, 50);
